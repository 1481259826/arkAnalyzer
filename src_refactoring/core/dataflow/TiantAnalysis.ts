...
export class TiantAnalysisChecker extends DataflowProblem<Value> {
    ...
    public setSources(methods: ArkMethod[]) {
        this.sources = methods;
    }

    public setSinks(methods: ArkMethod[]) {
        this.sinks = methods;
    }
...
// function findAliasObjects(local: Local, assignedStmt: Stmt): Local[] {
//     const cfg = assignedStmt.getCfg();
//     let declaringBlock: BasicBlock;
//     let index = -1;
//     for (const block of cfg!.getBlocks()) {
//         for (let i = 0; i < block.getStmts().length; i++) {
//             const stmt = block.getStmts()[i]
//             if (stmt == assignedStmt) {
//                 declaringBlock = block;
//                 index = i;
//                 break;
//             }
//         }
...
//         }
//     }
//     const aliasObjects: Local[] = [];
//     const otherLocals: Local[] = [];
//     const defStmts: Stmt[] = [];
//     // 判断本block之前有无对应def
//     for (let i = index - 1; i >= 0; i--) {
//         const beforeStmt = declaringBlock!.getStmts()[i];
//         if (beforeStmt.getDef() && beforeStmt.getDef() == local) {
//             defStmts.push(beforeStmt);
//             break;
//         }
//         if (beforeStmt instanceof ArkAssignStmt && beforeStmt.getDef() instanceof Local) {
//             const assigned = beforeStmt.getDef() as Local;
//             if (!aliasObjects.includes(assigned) && !otherLocals.includes(assigned)) {
//                 if (beforeStmt.getRightOp() == local) {
//                     aliasObjects.push(assigned);
//                 } else {
//                     otherLocals.push(assigned);
//                 }
//             }
//         }
//     }
...
//         for (const predecessor of declaringBlock!.getPredecessors()) {
